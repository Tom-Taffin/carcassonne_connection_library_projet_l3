...
/**
 * Command representing a play action in the game's interaction system.
 * This command is identified by the keyword "PLAYS" and follows the format: id PLAYS.
 * Upon reception, it triggers a view update via the updateOnPlay method.
 * @param <V> the type of spectator view this command operates on, must extend SpectatorView
 */
public class PlayCommand<V extends SpectatorView<?>> extends AbstractCommand<V> {

    /**
     * Constructs a PlayCommand and sets its keyword to "PLAYS".
     */
    public PlayCommand(){
        this.keyword = "PLAYS";
    }
    ...
    /**
     * Builds the string representation of the PLAYS command to be sent over the network.
     * @param id the identifier of the entity issuing the play
     * @param params must be null or empty, as the PLAYS command takes no parameters
     * @return the formatted PLAYS command string ready to be transmitted
