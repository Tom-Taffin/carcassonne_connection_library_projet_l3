...
/**
 * Command representing an enter action in the game's interaction system.
 * This command is identified by the keyword "ENTERS" and follows the format: id ENTERS.
 * Upon reception, it triggers a view update via the updateOnEnter method.
 * @param <V> the type of spectator view this command operates on, must extend SpectatorView
 */
public class EnterCommand<V extends SpectatorView<?>> extends AbstractCommand<V> {

    /**
     * Constructs an EnterCommand and sets its keyword to "ENTERS".
     */
    public EnterCommand(){
        this.keyword = "ENTERS";
    }
    ...
    /**
     * Builds the string representation of the ENTERS command to be sent over the network.
     * @param id the identifier of the entity issuing the enter
     * @param params must be null or empty, as the ENTERS command takes no parameters
     * @return the formatted ENTERS command string ready to be transmitted
